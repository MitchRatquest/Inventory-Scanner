...
with open('lookup-Hyperlinked.txt','r+') as b:	#open your lookup table
	lookups = b.readlines()
...
for palletrange in range(len(palletsplit)+1):
	try:
		...
		print finaloutput
		with open('manifest_hyperlinked.csv', 'w+') as dango: #this runs wher your list index is out of range(end of pallet list)
